...
void *mymalloc(size_t size) {
  ...
  while (curr != NULL) {
    if (!curr->pdata->is_allocated && (size_t) curr->pdata->val >= size) { // found a large enough block
      ...
      } else if (curr->pdata->val < min_size) {
        // update min_size
        min_size = curr->pdata->val;

        // update best_fit pointer
        best_fit = curr;
  ...
  if (best_fit != NULL) {
    ...
    newData->val = best_fit->pdata->val - size; // remaining free memory
    newData->is_allocated = false;
    newData->start_index = best_fit->pdata->start_index + size;

    TNode *newNode = make_node(best_fit->pdata->start_index + size, newData);
   
    // update info of the curr node
    best_fit->pdata->val = size; // allocate size memory to the curr free node
    best_fit->pdata->is_allocated = true;
    
    // insert new node to end of llist
    insert_node(&_memlist, newNode, ASCENDING);
    return (void*)&_heap[best_fit->pdata->start_index]; 
  }
  return NULL;
}
